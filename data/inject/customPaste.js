...
async function performPasteByTyping() {
    ...
    try {
        ...
        if (!clipText && customClip) {
            clipText = customClip;
            clipboardSource = 'neoExamShieldClipboard';
            console.log('[PasteByTyping] Using neoExamShieldClipboard:', clipText.substring(0, 100));
        }
...
async function performDragDropPaste() {
    ...
    try {
        ...
        if (!clipText && customClip) {
            clipText = customClip;
            clipboardSource = 'neoExamShieldClipboard';
            console.log('[DragDropPaste] Using neoExamShieldClipboard:', clipText.substring(0, 100));
        }
    ...
    if (ctrlKey && !event.shiftKey && !event.altKey && (event.key === 'V' || event.key === 'v')) {
        ...
        if (activeElement && (activeElement.isContentEditable || activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA')) {
            ...
            try {
                ...
                if (!clipText && customClip) {
                    clipText = customClip;
                    console.log('[Paste] Using neoExamShieldClipboard');
                }
